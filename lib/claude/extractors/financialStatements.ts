...
複数ページにまたがる可能性があるため、テキストに含まれる書類種別（P/L、販管費内訳、B/S）をすべて検出してください。
和暦は西暦に変換し、金額はカンマを除去し数値のみにしてください（単位は円）。不明な項目は null としてください。`;

export const USER_PROMPT = `以下のOCRテキストは会社の決算書です（損益計算書・販売費及び一般管理費内訳・貸借対照表）。
以下の項目を抽出してください：

- document_types_detected: テキストに含まれる書類種別の配列（"PL"=損益計算書, "SGA"=販管費内訳, "BS"=貸借対照表, "unknown"=判別不能）
- company_name: 会社名
